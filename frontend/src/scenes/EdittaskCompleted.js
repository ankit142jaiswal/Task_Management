...
function EdittaskCompleted() {
    ...
    const showdata = () => {
        ...
        if (window.completedList[location.state.index] != []) {
            setTitle(window.completedList[location.state.index].title);
            setDescription(window.completedList[location.state.index].description);
            setStartDate(window.completedList[location.state.index].startdate);
            setEndDate(window.completedList[location.state.index].enddate);
            setPriority(window.completedList[location.state.index].priority);
            setUploadfile(window.completedList[location.state.index].uploadfile);
            setDepartment(window.completedList[location.state.index].department);
            setEmployee(window.completedList[location.state.index].employee);
    ...
    const collectData = (e) => {
        ...
        if (location.state.index > -1 && location.state.index < window.completedList.length) {
            window.completedList.splice(location.state.index, 1, data);
            window.alert("Task has Updated Successfully !!")
            console.log(window.completedList);
            ...
            <div className='row'>
                ...
                <div className='col-11'>
                    <div className='mb-3 row p-3 ' style={{ fontSize: "larger", backgroundColor: "white", fontWeight: "bolder", boxShadow: "0 0 10px 1px rgba(0, 0, 0, 0.2)", borderRadius: "5px" }}>
                        <h1 style={{ textAlign: "center" }}>Edit Completed Task </h1>
                    </div>
                    <form>
...
export default EdittaskCompleted