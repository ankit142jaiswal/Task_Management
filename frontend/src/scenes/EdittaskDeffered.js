...
function EdittaskDeffered() {
    ...
    const showdata = () => {
        ...
        if (window.defferedList[location.state.index] != []) {
            setTitle(window.defferedList[location.state.index].title);
            setDescription(window.defferedList[location.state.index].description);
            setStartDate(window.defferedList[location.state.index].startdate);
            setEndDate(window.defferedList[location.state.index].enddate);
            setPriority(window.defferedList[location.state.index].priority);
            setUploadfile(window.defferedList[location.state.index].uploadfile);
            setDepartment(window.defferedList[location.state.index].department);
            setEmployee(window.defferedList[location.state.index].employee);
    ...
    const collectData = (e) => {
        ...
        if (location.state.index > -1 && location.state.index < window.defferedList.length) {
            window.defferedList.splice(location.state.index, 1, data);
            window.alert("Task has Updated Successfully !!")
            console.log(window.defferedList);
            ...
            <div className='row'>
                ...
                <div className='col-11'>
                    <div className='mb-3 row p-3 ' style={{ fontSize: "larger", backgroundColor: "white", fontWeight: "bolder", boxShadow: "0 0 10px 1px rgba(0, 0, 0, 0.2)", borderRadius: "5px" }}>
                        <h1 style={{ textAlign: "center" }}>Edit Pending Task </h1>
                    </div>
                    <form>
...
export default EdittaskDeffered